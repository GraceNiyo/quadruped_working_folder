...
base_data_dir = '../all_data/single_leg_experiment/single_leg_model/debugging_muscle_spindle/alpha_to_gamma_drive'
...
with mujoco.viewer.launch_passive(model, data, show_left_ui=False, show_right_ui = True) as viewer:
    ...
    for f in force_vector:
            ...
            if current_sim_time_relative < cumulative_times[0]: # Phase 1: 0 to 2 seconds
                ...
                for m in range(model.nu): # Calculate Ia feedback using the spindle model
                    ...
                    Ia_feedback, II_feedback = gamma_driven_spindle_model_(
                        ...
                        gamma_dynamic= gamma_dynamic[m] * collateral_input[m],
                        gamma_static= gamma_static[m] * collateral_input[m]
                    )
                    Ia_a[m] = Ia_feedback
                    II_a[m] = II_feedback

                collateral_input = np.clip(collateral_input + Ia_a + II_a, 0, 1)  # Update collateral input based on Ia and II feedback
        ...
        base_filename = f"indipendent_drive_force_{abs(f)}"
            # save each data array to a separate .txt file
        for data_key, data_list in run_data.items():
            if data_key in ['force', 'gamma_dynamic', 'gamma_static']:
                continue
